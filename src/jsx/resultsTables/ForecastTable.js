...
const ForecastTable = (adjustedTimes) => {
    ...
    const windHeaderText = <span className={'clickableHeaderCell'}>{showGusts ? 'Wind gust' : 'Wind speed'}</span>;
    ...
    const temperatureHeaderText = <span className={'clickableHeaderCell'}>{showApparentTemp ? "Feels like" : <Icon icon="temperature"/>}</span>;
            ...
            return (
                ...
                {forecast.map((point,index) =>
                    <tr key={point.time+Math.random().toString(10)}
                        ...
                        <td className='timeCell'><Time time={index === forecast.length-1 ? null : getAdjustedTime(point,index,adjustedTimes)}/></td>
                ...
                <HTMLTable compact={true} striped bordered interactive style={{ fontSize: "12px", "borderSpacing": "0px"}}>
                    <thead>
                        <tr>
                            ...
                            <th id={'temp'} className={'clickableHeaderCell'} onClick={toggleApparentDisplay} style={{ cursor: "pointer" }}>{temperatureHeader}</th>
                            ...
                            <th className={'clickableHeaderCell'} onClick={toggleAqi} style={{ cursor: "pointer" }} id={'aqi'}>
                            <Tooltip content={'Air quality shows current conditions, not forecasted. Click to toggle for the next forecast requested'} placement={'top'}>
                                <span className={fetchAqi?'largerClickableHeaderCell':'largerStruckClickableHeaderCell'}>AQI</span></Tooltip>
                            </th>
                            <th id={'wind'} onClick={toggleGustDisplay} style={{ cursor: "pointer" }}>{windHeader}</th>
                            <MediaQuery minWidth={501}>
